...
class iterator{
    /**
     * 当只有一个数据时，prev为false表示为顶部
     * 当只有一个数据时，next为false表示在尾部
     */
    prev = undefined
    next = undefined
...
class LIRS {
    ...
    get(key, ifReturnNode = false){
        ...
        switch(node.getType()){
            case LIRS_Type.LIR:
                ...
                //移动到顶端，不在顶端的情况下
                if(node !== this.s_head){ this.#MoveTop(node,true) }
                //节点类型设置为LIR
            ...
            case LIRS_Type.HIR:
                ...
                //不在s中，设置为HIR，分别放到s和q的顶端
                    ...
                    //移动到顶端
                    if(node !== this.q_head){ this.#MoveTop(node,false); }
                }
                break;
            //NHIR只可能出现在S中
            case LIRS_Type.NHIR:
    ...
    #Push(node,toS){
        if(toS){
            ...
            this.s_head = node
            //原头部节点prev连接新的节点
            if(headNode) {headNode.s.prev = node }
            else{
                //不存在头节点,说明是第一个数据，则尾部也为新节点
                this.s_tail = node
            }
            //新节点next连接原头节点
            node.s.prev = false //prev为false表示在顶部
            ...
            //新节点设为头部
            this.q_head = node
            if(headNode) { headNode.q.prev = node }
            else{
                //不存在头节点,说明是第一个数据，则尾部也为新节点
                this.q_tail = node
            }
            //新节点next连接原头节点
            node.q.prev = false //prev为false表示在顶部
            node.q.next = headNode || false //next为false表示在底部
            //q队列节点数+1
            this.q_size ++;
        }
