    ...
    */
    async query(sql,config){
        ...
        if( cahceEnable ){//是否开启了二级缓存
            let r = this.modelManager.getCache(config.model.tablename)
            if(r)
                return r;
        }
        //一级缓存(必开)
        ...
        result =  await query.query(sql).then((value)=>{
            ...
            if( cahceEnable ){//是否开启了二级缓存
                if(config.type == QueryTypes.SELECT)
                    this.modelManager.setCache(config.model.tablename,sql,result)
                else
                    this.modelManager.clearCache(config.model.tablename)
            }
            //处理一级缓存
            if(config.type == QueryTypes.SELECT)
...
module.exports.SqlOperation = SqlOperation
