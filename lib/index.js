...
const { hasEvent, awaitData, queryData } = require('./utils/event');
...
class Haorm{
        ...
        if(typeof type == 'string'){
            switch(type.toLocaleLowerCase()){
                case 'mysql':
                    ...
                    break;
                default:
                    throw new Error('not support the database type');
            }
        }
    ...
    */
    async query(sql,config){
        ...
        //存在事件，等待返回数据
        if(hasEvent(sql)){
            result = await awaitData(sql);
        }else{
            let queryFunc = async (sql,config) => {
                //真正的查询
                ...
                let result =  await query.query(sql).then((value)=>{
                    return value;
                }).catch((err)=>{
            ...
            //使用事件，占位查询，防止缓存击穿
            result = await queryData(sql,config,queryFunc).then((value)=>{
                //处理二级缓存
                if( cahceEnable ){//是否开启了二级缓存
