...
class MysqlQueryGenerator extends AbstractQueryGenerator{
    ...
    getFieldsFromStruct(arr,struct){
        if(!Array.isArray(arr))
            throw new Error('getFieldsFromStruct paramseters need type Array')
        let obj = {}
        arr.forEach(e =>{
    ...
    */
    select(tablename,struct,conditions,primaryKey = undefined,num = -1){
        ...
        //如果没有条件就不加空格
        let sqlStr = `SELECT ${columns} ${from}${conditionStr}`
        return sqlStr;
    }
    ...
    */
    update(tablename,struct,params,conditions){
        ...
        struct = this._ignoreField(struct,QueryTypes.UPDATE);
        let updateSet = [];//修改的值
        ...
        for(let [key,value] of Object.entries(params)){
            ...
            if(value){
                updateSet.push(`${this.escapeId(struct[key].fieldName)} = ${this.escape(value)}`)
            }
        }
    ...
    */
    ...
    _count(struct,counts,num){
        let str = [];
        let tb = this._label_table(num);
        counts.forEach(item => {
            let distinct = item.distinct ? 'DISTINCT' : ''
            if(item.key == 1 || item.key == '*' || item.key == `${tb}*`){
                str.push(`COUNT(${distinct}${item.key}) AS ${item.tag}`)
                return;
            }
            if(struct.hasOwnProperty(item.key))
                str.push(`COUNT(${distinct}${tb}${this.escapeId(struct[item.key].fieldName)}) AS ${item.tag}`)
        })
        return str.join(',')
    }
        ...
        if(conditions && conditions.join){
            conditions.join.forEach(item =>{
                if(!item.ignore){
                    ...
                    let joinColumns = this.columns(joinStruct,++num,true);
                    if(joinColumns != '')
    ...
    /* 按顺序执行
        {
            ...
            join:[{
                ...
                select:['id','name'](选择的字段对应的模型结构key)
                conditions:{
                    where:{
                        id : { value : 1 , 'symbol' : '='},
    ...
    */
    ...
    _join(struct , condition , num = -1){
        ...
        let targetParams = this.getFieldsFromStruct(condition.select,targetStruct) //连接表需要查询的字段参数结构（struct里面取）
        
        str += this.select(targetTablename,targetParams,targetCondition,targetPrimaryKey)
        let label = this._label(num)
        str += `)${label} `
        if(condition.on)
