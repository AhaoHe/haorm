...
class MysqlQueryGenerator extends AbstractQueryGenerator{
    ...
    */
    select(tablename,struct,conditions,primaryKey = undefined,num = -1){
        ...
        //需要优先，以为会改变列
        let conditionStr = conditions.define ? this.condition(struct,conditions.define,num) : this.condition(struct,conditions,num)
        conditionStr = conditionStr == '' ? '' : ` ${conditionStr}`
        let from = this.from(tablename,num);
        //获取列
        conditions.fields = conditions.fields == '*' || !conditions.fields ? Object.keys(structTmp) : conditions.fields;
    ...
    */
    insert(tablename,struct,params){
        ...
        for(let key in struct){
            this.validation.verify(struct[key],params[key]);
        }
    ...
    */
    update(tablename,struct,params,conditions){
        ...
        let updateSet = [];//修改的值
        let conditionStr = this.condition_one(struct,conditions,['where','limit']);
        //如果没有条件就不加空格
        conditionStr = conditionStr == '' ? '' : ` ${conditionStr}`
        for(let [key,value] of Object.entries(params)){
            ...
            if(struct[key].primarykey){
                ...
                conditionStr = conditionStr == '' ? ` WHERE ${struct[key].fieldName} = ${this.escape(value)}` : `${conditionStr} AND ${struct[key].fieldName} = ${this.escape(value)}`;
                continue;
            }
    ...
    _statistics(struct,item,num = -1){
        if(Array.isArray(item) && item.length == 2){
            ...
            statistics.forEach(item => {
                ...
                if(prefix == 'COUNT' && 
                    (item.key == 1 || item.key == '*' || item.key == `${tb}*`)
                ){
    ...
    */
     ...
     * 只选择oneName一种条件 
     */
    condition_one(struct,conditions,oneName,num = -1){
        // let conditionTemp;
        // if(typeof conditions == 'object' && conditions.hasOwnProperty(oneName)){
        //     conditionTemp = {}
        //     conditionTemp[oneName] = conditions[oneName]
        // }else{
        //     conditionTemp = conditions
        // }
        // let conditionStr = `${this.condition(struct,conditionTemp,num)}`;
        // return conditionStr;
        let conditionStr = [];
        const type = typeof conditions;
        if(type == 'string'){
            conditionStr.push(
                this.condition(struct,conditions,num)
            );
        }
        else if(type == 'object'){
            oneName = Array.isArray(oneName) ? oneName : [oneName];
            oneName.forEach(item =>{
                let conditionTemp = {};
                if(!conditions[item]) return;
                conditionTemp[item] = conditions[item]
                let c = this.condition(struct,conditionTemp,num);
                if(c != '')
                    conditionStr.push( c );
            })
        }
    ...
    */
            ...
            for(let [key,value] of Object.entries(condition)){
                if(key == '$and'){
                    ...
                    if(struct.hasOwnProperty(key) && struct[key].fieldName){
                        ...
                        value = typeof value == 'object' ? value : {value:value,symbol:'='};
                        conditionStr.push(
    ...
    _join(struct , condition , num = -1){
        ...
        //纠错，如果用户忘记在fields中加入on的字段。
        const isArr = Array.isArray(targetCondition.fields);
        if(isArr && targetCondition.fields.length == 0) condition.ignore = true;
        if(Array.isArray(condition.on)){
            let o = {};
            for(let val of condition.on){
                if(typeof val != 'object' || !val.key) throw new Error("When On is Array,params type need be oject and include 'key'!")
                if(isArr && targetCondition.fields.indexOf(val.key) == -1){
                    targetCondition.fields.push(val.key);
                }else if(typeof targetCondition.fields == 'object' && Array.isArray(targetCondition.fields.exclude)){
                    let index = targetCondition.fields.exclude.indexOf(val.key);
                    if(index != -1) targetCondition.fields.exclude[index] = undefined;
                }
                //改为对象
                o[val.key] = {symbol: val.symbol}
                if(val.targetKey) o[val.key]['targetKey'] = val.targetKey;
                else if (val.value) o[val.key]['value'] = val.value;
            }
            condition.on = o;
        }else if(condition.on){
            if(typeof condition.on != 'object') throw new Error("On type need be object or array!")
            for(let key in condition.on){
                if(!key) throw new Error("When On is Array,params need include 'key'!")
                if(isArr && targetCondition.fields.indexOf(key) == -1){
                    targetCondition.fields.push(key);
                }else if(typeof targetCondition.fields == 'object' && Array.isArray(targetCondition.fields.exclude)){
                    let index = targetCondition.fields.exclude.indexOf(key);
                    if(index != -1) targetCondition.fields.exclude[index] = undefined;
                }
            }
        }else if(condition.using){
            condition.using = Array.isArray(condition.using) ? condition.using : [condition.using]
            for(let key of condition.using){
                if(typeof key != 'string') throw new Error("Using params type need be string!")
                if(isArr && targetCondition.fields.indexOf(key) == -1){
                    targetCondition.fields.push(key);
                }else if(typeof targetCondition.fields == 'object' && Array.isArray(targetCondition.fields.exclude)){
                    let index = targetCondition.fields.exclude.indexOf(key);
        ...
        if(condition.on)
            ...
            throw new Error("Table join needs 'on' or 'using'.")
        }
        return str
    }
   ...
   _on(conditionOn , originalStruct,targetParams,originalNum , targetNum){
        ...
        let conditionStr = this._on_conditions(conditionOn , originalStruct,targetParams,originalNum , targetNum)
        if(conditionStr == '')
    ...
    _on_conditions(conditionOn , originalStruct,targetParams,originalNum , targetNum){
        let conditionStr = []
        let index = 0;
        // if(Array.isArray(conditionOn)){
        //     //[ {key:'id' ,symbol:'=' , value:'1'} ]
        //     for(let obj of conditionOn){
        //         //判断是否存在该字段的对象映射
        //         let key  = obj.key;
        //         let targetKey = obj.targetKey;
        //         let symbol = obj.symbol;
        //         let value = obj.value;
        ...
        //             if(value.index < 0) obj.index = 0;
        //             originalNum = obj.index;
        //         }
        //         //真正处理的逻辑
        //         key = `${this._label_table(targetNum)}${this.escapeId(key)}`;
        //         let tempStr = '';
        //         if(targetKey && originalStruct.hasOwnProperty(targetKey)){
        //             let originalKey = `${this._label_table(originalNum)}${this.escapeId(originalStruct[targetKey].fieldName)}`;
        //             tempStr = this.condition_symbol(undefined,originalKey,symbol,key)
        //             if(tempStr != ''){
        ...
        //             }
        //         }else if(value){
        //             tempStr = this.condition_symbol(undefined,key,symbol,this.escape(value))
        //             if(tempStr != ''){
        //                 conditionStr.push(tempStr)
        //                 index++;
        //                 continue;
        ...
        for(let [key,value] of Object.entries(conditionOn)){
            if(key == '$and'){
                ...
                let andConditions = [];
                value.forEach(item=>{
                    let str = this._on_conditions(item,originalStruct,targetParams,originalNum , targetNum);
                    let andLength = str.length;
                    let andStr = str.join(' AND ')
                    andConditions.push(andLength == 1 ? andStr :`(${andStr})`);
                })
            ...
            if(value.hasOwnProperty('value')){//条件是等于值
    ...
    */
    ...
    _brackets(struct,key,symbol,value){
        if(!Array.isArray(value) && typeof value == 'object'){//值为子查询
            this.select(value.tablename,value.struct,value.conditions)
        }
